...
func (t *vitessTx) UpdateObjectTransition(ctx context.Context, objectID string, storageClass string, transitionedAt int64, transitionedRef string) error {
	query := `
		UPDATE objects
	...
	result, err := t.tx.ExecContext(ctx, query, storageClass, transitionedAt, transitionedRef, objectID)
	if err != nil {
...
func (t *vitessTx) CountBuckets(ctx context.Context) (int64, error) {
	var count int64
...
func (t *vitessTx) IncrementChunkRefCount(ctx context.Context, chunkID string, size int64) error {
	now := timeNowNano()
	_, err := t.tx.ExecContext(ctx, `
...
func (t *vitessTx) DecrementChunkRefCount(ctx context.Context, chunkID string) error {
	now := timeNowNano()
	result, err := t.tx.ExecContext(ctx, `
...
func (t *vitessTx) IncrementChunkRefCountBatch(ctx context.Context, chunks []db.ChunkInfo) error {
...
func (t *vitessTx) DecrementChunkRefCountBatch(ctx context.Context, chunkIDs []string) error {
...
func (t *vitessTx) GetChunkRefCount(ctx context.Context, chunkID string) (int, error) {
	var refCount int
	err := t.tx.QueryRowContext(ctx, `
...
func (t *vitessTx) AddChunkReplica(ctx context.Context, chunkID, serverID, backendID string) error {
	now := timeNowNano()
	_, err := t.tx.ExecContext(ctx, `
...
func (t *vitessTx) RemoveChunkReplica(ctx context.Context, chunkID, serverID string) error {
	_, err := t.tx.ExecContext(ctx, `
...
func (t *vitessTx) GetChunkReplicas(ctx context.Context, chunkID string) ([]db.ReplicaInfo, error) {
...
func (t *vitessTx) GetChunksByServer(ctx context.Context, serverID string) ([]string, error) {
	rows, err := t.tx.QueryContext(ctx, `
...
func (t *vitessTx) GetZeroRefChunks(ctx context.Context, olderThan time.Time, limit int) ([]db.ZeroRefChunk, error) {
...
func (t *vitessTx) DeleteChunkRegistry(ctx context.Context, chunkID string) error {
	_, err := t.tx.ExecContext(ctx, `
...
// timeNowNano returns current time in nanoseconds for timestamp fields
